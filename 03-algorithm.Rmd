...
<!-- Motivation to map -->
It is common to present geospatial data on a choropleth map display. These maps show the landmass with administrative boundaries and can highlight the geographic patterns in geospatially related statistics [@SAMGIS].
In many countries, residents are increasingly gathering to live around major cities.
The division of the population into approximately equal population areas results in dramatically different square meterage of the geographic areas. The geographic units are usually administrative, such as states or counties. The units are filled with colour to represent the value of the statistic for the unit [@EI].
...
population is the statistic of interest, the result is a population cartogram. They fail to preserve a recognisable display due to the difference in size of metropolitan and rural areas [@ACTUC], [@GOINO].
...
The unique shapes of boundaries can be helpful for orienting users but may not contribute to their understanding of the spatial disease distribution as many of the communities are not visible in a choropleth display [@TVSSS]. This presents an opportunity for explicit map transformations to improve communication of distributions [@CBATCC]. In figure \ref{fig:qld_grid} the choropleth map underlies the alternative maps to allow for comparisons to be made between the alternatives.
...
<!-- knitr::include_graphics("figures/03-algorithm/aus_ggchoro.png") -->
<!-- ``` -->
...
<!-- knitr::include_graphics("figures/03-algorithm/aus_gghexmap.png") -->
<!-- ``` -->
...
The algorithm is named for the **Trigona carbonaria** bee species. Native to northern and eastern Australia this stingless species builds flat layers of hexagonal brood cells, spiralling out from a central point [@PH]. This hive design inspired the use of multiple focal points in the algorithm, meaning individual spirals can be built outward from various points on the geographic map base.
...
\caption{\label{fig:sugarbag_horizontal}The functions called within `create_hexmap` or directly by a user to create a hexagon tile map.}
\end{figure}


#### User choices

The`create_hexmap` function allows a user to begin using the algorithm with only two inputs, the shape file, and the id column. The id column should uniquely identify each geographic unit in the shape file.
The`create_hexmap` function will derive the centroids of the geographic units, calculate a grid of possible hexagon locations, and allocate each geographic unit to a single hexagon.
A user can then tweak the parameters of the hexagon map using the additional arguments to the `create_hexmap` function, these options may affect the speed of the creation. Optional arguments allow the user to control: 
...
tribble(
  ~parameter,        ~create_centroids, ~create_grid, ~allocate,
   "shp",            "essential",       "",           "",
   "sf_id",          "essential",       "",           "optional",
   "buffer_dist",    "",                "essential",  "",
   "centroids",      "",                "essential",  "essential",
   "hex_size",       "",                "essential",  "essential", 
   "focal_points",   "",                "",           "optional",
   "hex_grid",       "",                "",           "essential",
   "hex_filter",     "",                "",           "essential",
   "use_neighbours", "",                "",           "optional",
   "width",          "",                "",           "essential",
   "verbose",        "optional",        "optional",   "optional"
  ) %>% 
  knitr::kable(., format = "latex", booktabs = TRUE,
    linesep = c("", "\\addlinespace", "", "\\addlinespace",""),
    caption = "The parameters used for sugarbag functions.") %>% 
