...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# figures in the lit review paper}
\NormalTok{figs2 <-}\StringTok{ }\NormalTok{tibble}\OperatorTok{::}\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{Chapter =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"2"}\NormalTok{, }\DecValTok{6}\NormalTok{)),}
  \DataTypeTok{Figure =} \KeywordTok{c}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{6}\NormalTok{)),}
  \DataTypeTok{Caption =} \KeywordTok{c}\NormalTok{(}
    \StringTok{"A selection of choropleth cancer maps from online atlases that are publicly available."}\NormalTok{,}
    \StringTok{"Common alternatives to maps, showing the same information for the United States of America."}\NormalTok{,}
    \StringTok{"Two alternative displays, tile map (left) and geofaceted map (right), showing state age-adjusted rate of incidence for lung and bronchus in the USA."}\NormalTok{,}
    \StringTok{"Cartograms showing melanoma incidence in Australia."}\NormalTok{,}
    \StringTok{"Interactive controls of displays in publicly available choropleth cancer maps"}\NormalTok{,}
    \StringTok{"Two examples of advanced interactivity (and animation) in publicly available choropleth cancer maps."}\NormalTok{),}
  \DataTypeTok{Page =} \KeywordTok{c}\NormalTok{(}\DecValTok{12}\NormalTok{, }\DecValTok{20}\NormalTok{, }\DecValTok{24}\NormalTok{, }\DecValTok{26}\NormalTok{, }\DecValTok{28}\NormalTok{, }\DecValTok{29}\NormalTok{)}
\NormalTok{)}

\CommentTok{# figures in the algorithm chapter}
\NormalTok{figs3 <-}\StringTok{ }\NormalTok{tibble}\OperatorTok{::}\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{Chapter =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"3"}\NormalTok{, }\DecValTok{7}\NormalTok{)),}
  \DataTypeTok{Figure =} \KeywordTok{c}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{7}\NormalTok{)),}
  \DataTypeTok{Caption =} \KeywordTok{c}\NormalTok{(}
    \StringTok{"Grid points to create a tilegram."}\NormalTok{,}
    \StringTok{"All possible hexagon locations from the initial grid are shown with blue outlines."}\NormalTok{,}
    \StringTok{"Filter for grid points within a square, then circular, distance for those closest to the centroid."}\NormalTok{,}
    \StringTok{"A complete hexagon tile map of Tasmania"}\NormalTok{,}
    \StringTok{"The Australian Cancer Atlas data has determined the colour of each Statistical Area of Australian at Level 2"}\NormalTok{,}
    \StringTok{"A choropleth map of the Statistical Areas of Australia at Level 2."}\NormalTok{,}
    \StringTok{"A hexagon tile map of the Statistical Areas of Australia at Level 2."}\NormalTok{), }
  \DataTypeTok{Page =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{9}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\NormalTok{)}


\NormalTok{figs4 <-}\StringTok{ }\NormalTok{tibble}\OperatorTok{::}\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{Chapter =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"4"}\NormalTok{, }\DecValTok{7}\NormalTok{)),}
  \DataTypeTok{Figure =} \KeywordTok{c}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{7}\NormalTok{)),}
  \DataTypeTok{Caption =} \KeywordTok{c}\NormalTok{(}
    \StringTok{"A choropleth map of the smoothed average of liver cancer diagnoses for Australian males."}\NormalTok{,}
    \StringTok{"A hexagon tile map of the smoothed average of liver cancer diagnoses for Australian males."}\NormalTok{,}
    \StringTok{"This lineup of twelve hexagon tile map displays contains one map with a real population related structure."}\NormalTok{,}
    \StringTok{"The experimental design used in the visual inference study."}\NormalTok{,}
    \StringTok{"The detection rates achieved by participants are contrasted when viewing the four replicates of the three trend models"}\NormalTok{,}
    \StringTok{"The distribution of the time taken (seconds) to submit a response for each combination of trend, whether the data plot was detected, and type of display, shown using horizontally jittered dotplots."}\NormalTok{,}
    \StringTok{"The amount of times each level of certainty was chosen by participants when viewing hexagon tile map or choropleth displays."}\NormalTok{), }
  \DataTypeTok{Page =} \KeywordTok{c}\NormalTok{(}\DecValTok{12}\NormalTok{, }\DecValTok{20}\NormalTok{, }\DecValTok{23}\NormalTok{, }\DecValTok{25}\NormalTok{, }\DecValTok{28}\NormalTok{, }\DecValTok{29}\NormalTok{, }\DecValTok{30}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tabs2 <-}\StringTok{ }\NormalTok{tibble}\OperatorTok{::}\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{Chapter =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"2"}\NormalTok{, }\DecValTok{6}\NormalTok{)),}
  \DataTypeTok{Table =} \KeywordTok{c}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{6}\NormalTok{)),}
  \DataTypeTok{Caption =} \KeywordTok{c}\NormalTok{(}
    \StringTok{"A selection of choropleth cancer maps from online atlases that are publicly available."}\NormalTok{,}
    \StringTok{"Common alternatives to maps, showing the same information for the United States of America."}\NormalTok{,}
    \StringTok{"Two alternative displays, tile map (left) and geofaceted map (right), showing state age-adjusted rate of incidence for lung and bronchus in the USA."}\NormalTok{,}
